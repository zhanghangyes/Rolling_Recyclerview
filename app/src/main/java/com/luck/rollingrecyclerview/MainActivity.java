        ...
        recyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                if (overallXScroll <= 0) {   //设置标题的背景颜色
                    llSearch.setBackgroundColor(getResources ().getColor (R.color.app_color)); //默认背景
                } else if (overallXScroll > 0 && overallXScroll <= height) { //滑动距离小于banner图的高度时，设置背景和字体颜色颜色透明度渐变
                    float scale = (float) overallXScroll / height;
                    float alpha = (255 * scale);
                    llSearch.setBackgroundColor(getResources ().getColor (R.color.color_work_money));
                } else {
                    llSearch.setBackgroundColor(getResources ().getColor (R.color.colorAccent));
                }
            }
        });
    }
